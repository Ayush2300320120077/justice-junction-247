...
// ── Catch-all 404 for unmatched API routes (M2 FIX) ─────────────────────────
app.use('/api/*', (req, res) => {
...
// ── Start server with eager DB connection (M1 FIX) ──────────────────────────
const PORT = process.env.PORT || 5000
connectDB()
  .then(() => {
    app.listen(PORT, () => console.log(`✓ Justice Junction API on port ${PORT} (${process.env.NODE_ENV || 'development'})`))
  })
  .catch(err => {
