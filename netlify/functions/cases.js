...
// POST A CASE UPDATE (lawyer only)
...
// GET UPDATES FOR A CASE (by booking ID)
router.get('/booking/:bookingId', async (req, res) => {
  try {
    await connectDB();
    const user = getUser(req);
    if (!user) return res.status(401).json({ error: 'Unauthorized' });
    const updates = await CaseUpdate.find({ booking: req.params.bookingId })
      .sort({ createdAt: 1 });
    res.json({ updates });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
// GET ALL UPDATES FOR LOGGED IN CLIENT
...
app.use('/.netlify/functions/cases', router);
