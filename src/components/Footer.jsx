          ...
          <div key={title} className="mobile-text-center">
            ...
            <ul style={s.list}>
              {links.map(([href, label]) => (
                <li key={label}><Link href={href} style={s.footLink} className="foot-link">{label}</Link></li>
              ))}
            </ul>
          </div>
        ))}
...
const s = {
  footer:{background:'#1A0D10',padding:'5rem 5vw 2rem',color:'#F5C4B3', position:'relative'},
