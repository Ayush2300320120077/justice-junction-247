...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && action === 'verify') {
      ...
      return res.json({ success: true, booking, message: 'Payment successful! Booking confirmed.' })
    }
    ...
    if (req.method === 'POST' && action === 'subscribe') {
      ...
      return res.json({ orderId: order.id, amount, currency: 'INR', keyId: process.env.RAZORPAY_KEY_ID, plan })
    }
    ...
    if (req.method === 'POST' && action === 'verify-subscription') {
      ...
      return res.json({ success: true, plan, message: `${PLAN_FEATURES[plan].name} plan activated!` })
    }
