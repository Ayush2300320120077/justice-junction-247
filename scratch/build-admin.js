...
const backupPath = path.join(__dirname, '../api/_routes/admin.js.backup');
const targetPath = path.join(__dirname, '../api/_routes/admin.js');

const backup = fs.readFileSync(backupPath, 'utf8');
const lines = backup.split('\n');

const loginStart = lines.findIndex(l => l.includes('// ── ADMIN ROUTE: LOGIN ──') || l.includes('const rateLimit = require(\'express-rate-limit\')'));
let loginEnd = lines.findIndex((l, i) => i > loginStart && l.includes('// ── REQUIRE ADMIN AUTH MIDDLEWARE ──'));
if (loginEnd === -1) loginEnd = lines.findIndex((l, i) => i > loginStart && l.includes('router.use(authMiddleware, requireAdmin)'));

// Include the rateLimit definition which might be right before login
const rateLimitStart = lines.findIndex(l => l.includes("const rateLimit = require('express-rate-limit');"));
const loginSectionLines = lines.slice(rateLimitStart !== -1 ? rateLimitStart : loginStart, loginEnd);

const aiLogsStart = lines.findIndex(l => l.includes('// ── AI EVALUATION & LOGS ──'));
const aiLogsEnd = lines.findIndex((l, i) => i > aiLogsStart && l.includes('app.use(\'/api/admin\', router);'));
const aiLogsLines = lines.slice(aiLogsStart, aiLogsEnd);

const newAdminContent = `require('dotenv').config();
...
router.post('/document-templates', asyncHandler(async (req, res) => {
  await connectDB();
...
router.put('/document-templates/:id', asyncHandler(async (req, res) => {
  await connectDB();
  const { title, category, description, fields, templateBody, isActive } = req.body;
...
fs.writeFileSync(targetPath, newAdminContent);
console.log('Successfully rewrote admin.js');
